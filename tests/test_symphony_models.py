...
class RoutingTests(unittest.TestCase):
    ...
    def test_turn_profile_preserves_policy_and_records_reason(self):
        ...
        prepared = json.loads(worker.prepare_request(json.dumps(request).encode(), self.project, self.root, self.cwd, router))["params"]
        self.assertEqual(("gpt-6-sol", "medium"), (prepared["model"], prepared["effort"]))
        self.assertEqual("never", prepared["approvalPolicy"])
        self.assertFalse(prepared["sandboxPolicy"]["networkAccess"])
        self.assertTrue(prepared["sandboxPolicy"]["excludeSlashTmp"])
        self.assertEqual([str(self.cwd), str(self.cwd / ".git")], prepared["sandboxPolicy"]["writableRoots"])
    ...
    def test_opt_in_configuration_and_catalog_preflight(self):
        config = project.setup(self.project, project_id="test", project_slug="test", setup_issue="TEST-1", repo_url="git@example.invalid:test.git", codex="codex", model_routing="auto")
