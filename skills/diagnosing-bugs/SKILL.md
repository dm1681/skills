...
10. **HITL bash script.** Interactive sessions only; workers record a blocker when human reproduction is required. Last resort. If a human must click, drive _them_ with `scripts/hitl-loop.template.sh` so the loop is still structured. Captured output feeds back to you.
...
After the Phase 1 completion criterion is met, follow [DIAGNOSIS-PHASES.md](DIAGNOSIS-PHASES.md) for reproduction, hypotheses, instrumentation, regression and cleanup.
