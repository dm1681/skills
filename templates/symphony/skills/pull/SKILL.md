...
name: pull
description:
  Pull latest origin/<base_branch> into the current local branch and resolve merge
...
4. Fetch latest refs:
   - `git fetch origin`
5. Sync the remote feature branch first if it exists after the fetch:
   - Read the current branch with `git branch --show-current` and check for
     `refs/remotes/origin/<current_branch>` using `git show-ref --verify`.
   - If present, run `git pull --ff-only origin "<current_branch>"`.
   - If absent, this is an unpublished issue branch; continue to the base merge.
