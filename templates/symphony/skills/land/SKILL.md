...
Read `.symphony-worker.json` for `base_branch` and `validation_command`; substitute the actual branch in command examples. Preserve project coding standards and use the configured validation command instead of upstream repository-specific checks.
...
- Do not yield to the user until the PR is merged; keep the watcher loop running
  unless blocked.
- Respect the repository's branch deletion policy after merge.
...
4. Check mergeability and conflicts against the configured base branch.
5. If conflicts exist, use the `pull` skill to fetch/merge `origin/<base_branch>` and
...
9. When all checks are green and review feedback is addressed, squash-merge
   ...
   when the repository's policy calls for it.
10. **Context guard:** Before implementing review feedback, confirm it does not
    conflict with the user’s stated intent or task context. If it conflicts,
    respond inline with a justification and record the conflict in the workpad and return to Human Review before changing code.
...
12. **Ambiguity gate:** When ambiguity blocks progress, use the clarification
    flow (record the unresolved decision in the workpad and return to Human Review). Do not
...
# Sweep existing feedback; no repository-specific review bot is required.
gh pr view --json reviews,comments
gh api repos/{owner}/{repo}/pulls/"$pr_number"/comments
# Address actionable feedback and confirm approval before continuing. Do not
# wait for a particular bot comment or interpret an absent bot as approval.
...
# Squash-merge after approval and successful required checks
...
- Where a repository posts `## Codex Review` issue comments, include them in
  the feedback sweep. No such bot or comment marker is required by this pilot.
- Do not enable auto-merge as a substitute for the workflow's approval and
  validation gates.
...
- Some repositories post automated Codex reviews as issue comments starting
  with `## Codex Review`. If present, treat them as feedback that must be
  acknowledged before merge. The human review for this pilot has no mandatory
  comment marker.
